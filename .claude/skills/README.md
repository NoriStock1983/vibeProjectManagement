...
### [code-quality-review](common/code-quality-review/SKILL.md) — コード品質レビュー＆リファクタリング

「Good Code, Bad Code」（Tom Long著）の6つの品質特性でコードをレビューし、リファクタリング案を合わせて出力する。

| 項目 | 内容 |
| ---- | ---- |
| **実行コマンド** | `/code-quality-review` または「コードレビューして」「good code bad codeの観点でみて」と話しかける |
| **出力先** | `vibeProjectManagement/docs/code-reviews/code-review_[対象名]_YYYY-MM-DD.md` |
| **入力** | レビュー対象コードのファイルパス or 貼り付けテキスト |

**できること:**

- Correct / Readable / Predictable / Robust / Testable / Maintainable の6軸でコードを評価
- 問題点を優先度（High/Medium/Low）付きで一覧化
- Highを中心にリファクタリング済みコードを生成し、変更意図を説明
- テンプレート ([assets/template.md](common/code-quality-review/assets/template.md)) に従ったフォーマットで出力
